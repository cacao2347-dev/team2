...
public class MyUtil
{
	...
	public int getPageCount(int numPerPage, int dataCount)
	{
		...
		// 91개의 게시물이면 10게시물당 1페이지라 쳤을때 9개의 게시물밖에 표시가 안되서
		// 페이지수 하나를 늘려주기 위해 pageCount를 한개 늘려준다
	...
	public String getIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		// -- 현재 페이지(이 페이지 기준으로 보여주는 숫자가 달라져야 하기 때문...)
		int page;
		int n;
		...
		if(listUrl.indexOf("?") != -1)	// 링크를 설정할 URL 에 『?』가 들어있으면...
			listUrl = listUrl + "&";
		else							// 링크를 설정할 URL 에 『?』가 들어있지 않으면...
			listUrl = listUrl + "?";
		//-- 예를 들어, 검색값이 존재하면 이미 request 값에
		//   search 값과 searchValue 값이 들어있는 상황이므로
		//   『&』를 붙여서 추가해 주어야 한다.
		
		// currentPageSetup = 표시할 첫 페이지 - 1 
		currentPageSetup = (currentPage / numPerBlock ) * numPerBlock;
		//-- 만약 현재 페이지 5페이지이고(currentPage=5)
		//   리스트 하단에 보여줄 페이지의 갯수가 10이면(numPerBlock=10)
		...
		// 10으로 딱 나눠떨어진다면 10 - 10 = 0 페이지로 페이지를 셋업 시켜준다.
		...
		// totalPage가 작을 경우 << 첫 페이지로 돌아가는 화살표가 굳이 필요없다.
		
		// Prev
		n = currentPage - numPerBlock;
		if( (totalPage > numPerBlock) && (currentPageSetup > 0) )
			strList.append(" <a href='" + listUrl + "=" + n + "'>Prev</a>");
		...
		//-- 『+1』 을 하는 이유는
        //   앞에서 currentPageSetup 에서 10을 가져왔따면
